...
int solution(vector<vector<int>> board, vector<int> moves) {
	int answer = 0, N = board.size(), catchedFriend;
	stack<int> ST;
	for (auto it : moves) {
		catchedFriend = 0;
		for (int i = 0; !catchedFriend && i < N; i++) {
			if (board[i][it - 1]) {
				catchedFriend = board[i][it - 1];
				board[i][it - 1] = 0;
			}
		}
		if (catchedFriend) {
			if (!ST.empty()) {
				if (ST.top() == catchedFriend) {
					answer += 2;
					ST.pop();
				}
				else ST.push(catchedFriend);
			}
			else ST.push(catchedFriend);
		}
	}
...
vector<int> createAnswer() {
	...
	for (int i = 0; i < tupleLength; i++) {
		int len = nTuple[i].length(), v = 0;
		for (int j = 0; j < len; j++) {
			if (nTuple[i][j] == ',' || nTuple[i][j] == '}') {
				if (!exist[v]) {
					ret.push_back(v);
					exist[v] = true;
					break;
...
int ans, bSizeList[8], bidSize, uidSize;
vector<int> matchList[8];
bool checkBit[40321];

void createMatchList(vector<string> &uid, vector<string> &bid) {
	for (int i = 0; i < bidSize; i++) {
		bSizeList[i] = bid[i].length();
		for (int j = 0; j < uidSize; j++) {
			int uLen = uid[j].length();
			if (uLen != bSizeList[i]) continue;
			bool match = true;
			for (int k = 0; match && k < uLen; k++) {
				if (bid[i][k] == '*') continue;
				if (uid[j][k] != bid[i][k]) match = false;
			}
			if (match) matchList[i].push_back(j);
		}
	}
}

void cntAnswerRec(int idx, int bit) {
	if (checkBit[bit]) return;
	checkBit[bit] = true;
	if (idx == bidSize) ans++;
	else {

		for (auto it : matchList[idx]) {
			if (checkBit[1 << it]) continue;
			cntAnswerRec(idx + 1, bit | (1 << it));
		}
	}
}

int solution(vector<string> user_id, vector<string> banned_id) {
	bidSize = banned_id.size();
	uidSize = user_id.size();
	createMatchList(user_id, banned_id);
...
long long finding(long long v) {
	if (Mp[v] == 0) {
		Mp[v] = v + 1;
		return v;
	}
	return Mp[v] = finding(Mp[v]);
}

vector<long long> solution(long long k, vector<long long> room_number) {
	for (auto it : room_number)
		answer.push_back(finding(it));
...
int solve(vector<int> &stones, int k) {
	...
	int len = stones.size();
	for (int i = 0, localMax; i < len; i++) {
		localMax = 0;
		while (st.top().v < stones[i]) {
			if (i - st.top().idx <= k)
				localMax = max(localMax, st.top().v);

			st.pop();
		}
		if (i - st.top().idx > k && localMax) 
			ret = min(ret, localMax);
